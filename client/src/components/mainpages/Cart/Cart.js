...
const Cart = () => {
  ...
  const orderAddressToBlockchain = async ({
    ...
    // If using metamask, prompt the user to send transactions with thsi account
    //const accounts = await window.ethereum.enable();
    // Retrieve the first account for send transactions to blockchain
    //const account = accounts[0];
    // Retrieve accounts link to the provider
    const accounts = await web3.eth.getAccounts();

    // Dynamically determine the gas fee
